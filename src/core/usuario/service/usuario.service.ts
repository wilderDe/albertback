      ...
      if (usuarioNuevo?.id) {
        ...
        if (usuarioNuevo.correoElectronico) {
          ...
          const data = {
            ...
            contenido: template,
            /*"adjunto": {"nombre": "adjuntoImportanteFinal.txt", "archivo": "data:text/plain;base64,aGVsbG8gd29ybGQ="},*/
            tipo: 1,
          ...
          try {
            ...
            this.logger.info(
              `Correo enviado con éxito a ${usuarioNuevo.correoElectronico}`
            )
          } catch (error) {
  ...
  async recuperarCuenta(recuperarCuentaDto: RecuperarCuentaDto) {
    ...
    if (usuario.correoElectronico) {
      ...
      try {
        ...
        //console.log(respuesta.data)
        this.logger.info(
          `Correo enviado con éxito a ${usuario.correoElectronico}`
        )
      } catch (error) {
        throw new Error('Error al hacer el post de enviar email')
      }
